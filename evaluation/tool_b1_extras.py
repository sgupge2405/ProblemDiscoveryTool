...
def _normalized_entropy_from_vector(vec: List[int]) -> float:
    """
    vec（カテゴリ合計スコアのベクトル）から正規化エントロピーを算出（0..1）
    - total=0 の場合は 0.0
    - p_i = v_i / sum(v)
    - H = -sum p_i log(p_i)
...
class ToolBExtrasOutputs:
    ...
    # sample_id × extra_id × category_score（横持ち）
    extra_detail_df: pd.DataFrame

    report: Dict[str, Any]


def run_tool_b_meaning_scores_extras(
    ...
    *,
    ...
    """
    ツールB1-extras（IR網羅性：意味カテゴリスコア + 正規化エントロピー）
    対象：
    - objective_card.extras（配列要素を1件ずつ評価対象として扱う）
    ...
    2) entropy_df：
       ...
       - activity_sum / activity_mean / activity_ratio / activity_saturation（補助）
    3) extra_detail_df：sample_id×extra_id×category_score（横持ち）

    sample_id：
    - ZIP名を sample_id として用いる（B1と同じ）
    ログ（追記式）：
    ...
    # 詳細（sample_id×extra_id×category）横持ち行
    ...
    for t in trials:
        ...
        extras_raw = t.objective_card.get("extras")

        # extras が「配列」以外で来ても最低限拾えるようにする
        ...
        for idx, item in enumerate(extras_list, start=1):
            text = normalize_text(item)
            if not text:
                continue

            extra_id = f"extras_{idx:02d}"

            out = labeler.label_meaning_scores(
                sample_id=sample_id,
                text_id=extra_id,   # jsonl上の識別子
                raw_text=text,
                meta=meta,
                out_jsonl=out_jsonl,
                err_jsonl=err_jsonl,
            ...
            row = {"sample_id": sample_id, "extra_id": extra_id}
    ...
    # (2) entropy（主）+ Activity補助 + entropy（参考）
    ...
    for _, row in category_sum_df.iterrows():
        ...
        vec_no_activity = [int(row[k]) for k in keys_no_activity]
        vec_all = [int(row[k]) for k in meaning_keys]

        h_no_activity = _normalized_entropy_from_vector(vec_no_activity)
        h_all = _normalized_entropy_from_vector(vec_all)

        activity_sum = int(row.get(activity_key, 0))
        activity_mean = float(activity_sum / max(1, len(trials)))  # B1のノリに合わせた“参考”値
        activity_ratio = float(activity_sum / total_score) if total_score > 0 else 0.0
        activity_saturation = float(activity_sum / 15.0)  # B1踏襲（目安）

        entropy_rows.append(
            {
                "sample_id": sample_id,
                "normalized_entropy_no_activity": float(h_no_activity),
                "normalized_entropy_all": float(h_all),
                "total_score": int(total_score),
                "activity_sum": int(activity_sum),
    ...
    extra_detail_df = pd.DataFrame(detail_rows)

    # category_sum_df に主指標を併記（B1と同じ“便利合体”）
    ...
    report = {
        "tool": "tool_b1_extras_coverage",
        "sample_id_policy": "zip_name",
        "target": "objective_card.extras (each item)",
        ...
        "notes": [
            ...
            "extra_detail_df: sample_id×extra_id×カテゴリスコア（横持ち）",
        ],
    }

    return ToolBExtrasOutputs(
