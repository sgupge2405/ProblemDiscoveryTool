...
from evaluation.tool_b1_extras import run_tool_b_meaning_scores_extras


st.set_page_config(page_title="評価ツールB1-extras（IR網羅性）", layout="wide")
st.title("評価実験ツールB1-extras（IR網羅性：意味カテゴリスコア＋正規化エントロピー）")

st.caption(
    "対象：objective_card.extras（配列要素を1件ずつ）／"
...
with st.expander("設定", expanded=True):
    ...
    default_out = f"outputs/tool_b1_extras_coverage/{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
if run_clicked:
    try:
        with st.status("評価を実行中…（LLM分類のため時間がかかります）", expanded=True) as status:
            st.write("extras抽出 → 正規化 → 意味カテゴリスコア付与 → 集計 → エントロピー（主/参考）＋Activity補助指標算出")
            res = run_tool_b_meaning_scores_extras(
                trials,
    ...
    st.subheader("extra_detail（sample_id×extra_id×category_score）")
    st.dataframe(res.extra_detail_df, use_container_width=True)
    ...
    with zipfile.ZipFile(zip_buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("tool_b1_extras_entropy.csv", res.entropy_df.to_csv(index=False, encoding="utf-8-sig"))
        zf.writestr("tool_b1_extras_category_sum.csv", res.category_sum_df.to_csv(index=False, encoding="utf-8-sig"))
        zf.writestr("tool_b1_extras_extra_detail.csv", res.extra_detail_df.to_csv(index=False, encoding="utf-8-sig"))
        zf.writestr("tool_b1_extras_report.json", json.dumps(res.report, ensure_ascii=False, indent=2))

    st.download_button(
        label="結果一式（ZIP）をダウンロード",
        data=zip_buf.getvalue(),
        file_name="tool_b1_extras_outputs_ir_coverage.zip",
